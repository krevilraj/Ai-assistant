...
function ai_tg_fix_img_src($html, $theme_dir = '')
{
    return preg_replace_callback(
        ...
        function ($m) use ($theme_dir) {
            ...
            // CRITICAL FIX: Decode HTML entities first to check for PHP code
            $decoded_src = html_entity_decode($src, ENT_QUOTES | ENT_HTML5, 'UTF-8');

            // If the src contains PHP code (after decoding entities), skip it
            if (strpos($decoded_src, '<?php') !== false ||
                strpos($decoded_src, 'echo') !== false ||
                strpos($decoded_src, 'get_stylesheet_directory_uri') !== false) {
                return $m[0];
            }
...
if (!function_exists('ai_tg_fix_inline_style_urls')) {
    function ai_tg_fix_inline_style_urls($html, $theme_dir = '') {
        return preg_replace_callback(
            '/\bstyle=(["\'])(.*?)\1/is', // Added 's' flag for multiline
            function ($m) use ($theme_dir) {
                $outer_quote = $m[1];
                $style = $m[2];

                // Decode HTML entities to check for PHP code
                $decoded_style = html_entity_decode($style, ENT_QUOTES | ENT_HTML5, 'UTF-8');

                // If style already contains PHP code, skip processing
                if (strpos($decoded_style, '<?php') !== false ||
                    strpos($decoded_style, 'echo') !== false ||
                    strpos($decoded_style, 'get_stylesheet_directory_uri') !== false) {
                    return $m[0];
                }
                ...
                $style = preg_replace_callback(
                    ...
                    function ($u) use ($theme_dir) {
                        ...
                        // Decode HTML entities to check for PHP code
                        $decoded_url = html_entity_decode($url, ENT_QUOTES | ENT_HTML5, 'UTF-8');

                        // If URL already contains PHP code, skip processing
                        if (strpos($decoded_url, '<?php') !== false ||
                            strpos($decoded_url, 'echo') !== false ||
                            strpos($decoded_url, 'get_stylesheet_directory_uri') !== false) {
                            return $u[0];
                        }
                        ...
                        // Only rewrite if file exists in theme
                        if ($theme_dir && ai_tg_theme_asset_exists($theme_dir, $normalized)) {
                            return "background-image: url('" . "<?php echo esc_url( get_stylesheet_directory_uri() ); ?>/" . $normalized . "')";
                        }

                        return $u[0]; // keep original
                    },
                    $style
                );
...
if (!function_exists('ai_tg_write_page_template_in_theme')) {
    function ai_tg_write_page_template_in_theme($theme_dir, $slug, $source_filename, $raw_content, $ext, $use_featured_image = false)
    {
        ...
        // IMPORTANT: Do featured image replacement ONLY if requested
...
function ai_tg_create_wp_pages_with_templates($rows, $page_status, $zip_saved_path, $theme_dir, $target_mode, $new_theme_slug_for_activation = '', $activate_theme = false)
{
    ...
    foreach ($rows as $row) {
        ...
        $use_featured = !empty($row['featured']) && (string)$row['featured'] === '1';
        $template_file = ai_tg_write_page_template_in_theme($theme_dir, $slug, $source_basename, $content, $ext, $use_featured);
...
?>
    <div class="wrap ai-theme-generator">
        ...
        <form id="ai-theme-generator-form" class="ai-tg-form" method="post" enctype="multipart/form-data">
            ...
            <input type="hidden" name="ai_tg_zip_saved" value="<?php echo esc_attr($zip_saved_path); ?>"
                   id="ai_tg_zip_saved">
            ...
            <div class="ai-tg-card">
                ...
                <div class="ai-tg-radio-row">
                    <label class="ai-tg-radio">
                        <input type="radio" name="ai_tg_target_mode"
                               value="existing" <?php checked($target_mode, 'existing'); ?>>
                ...
                <div class="ai-tg-new-theme-options">
                    ...
                    <label class="ai-tg-check">
                        <input type="checkbox" id="ai_tg_activate_theme" name="ai_tg_activate_theme"
                               value="1" <?php checked(true, true); ?>>
                        ...
                        <div class="ai-tg-card" style="margin-top:14px;">
                            ...
                            <div class="ai-field ai-tg-field">
                                ...
                                <select id="ai_tg_page_status" name="ai_tg_page_status">
                                    ...
                                    <option value="publish" selected>Publish</option> <!-- Add selected -->
                                </select>
                            ...
                            <div class="ai-tg-actions" style="margin-top:14px;">
                                <button type="submit" class="button button-primary ai-tg-btn"
                                        id="ai_tg_btn_create_pages">
                                    Create Selected WordPress Pages + Templates
                                </button>
                                <p class="description" style="margin:8px 0 0;">
                                    This will generate <code>page-{slug}.php</code> inside the theme root and attach it
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Defaults as before
            const createToggles = document.querySelectorAll('input[name^="ai_tg_pages"][name$="[create]"]');
            createToggles.forEach(toggle => toggle.checked = true);

            const pageStatusSelect = document.getElementById('ai_tg_page_status');
            if (pageStatusSelect) pageStatusSelect.value = 'publish';

            const activateThemeCheckbox = document.getElementById('ai_tg_activate_theme');
            if (activateThemeCheckbox) activateThemeCheckbox.checked = true;

            // Check if we just completed generation (look for success message)
            const successNotice = document.querySelector('.notice.notice-success');
            if (successNotice && successNotice.textContent.includes('Created')) {
                // Show popup after 1 second
                setTimeout(showHomepagePopup, 1000);
            }
        });
        ...
        function showHomepagePopup() {
            // Create popup HTML
            const popupHTML = `
        <div id="ai-homepage-popup" style="
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
        ...
        ">
            <div style="
                ...
                overflow-y: auto;
            ">
                <h2>Set Homepage & Reset Permalinks</h2>
                <p>Please select which page should be set as the homepage:</p>

                <div style="margin: 20px 0;">
                    <label for="ai-homepage-select" style="display: block; margin-bottom: 10px; font-weight: bold;">
                        Select Homepage:
                    </label>
                    <select id="ai-homepage-select" style="width: 100%; padding: 10px; margin-bottom: 20px;">
                        <option value="">-- Select a page --</option>
                        <?php
            // Get all pages created in this session
            if (isset($created) && is_array($created)) {
                foreach ($created as $page) {
                    echo '<option value="' . esc_attr($page['id']) . '">' .
                        esc_html($page['title']) . ' (ID: ' . $page['id'] . ')</option>';
                }
            }
            ?>
                    </select>
                ...
                <div style="margin: 20px 0; padding: 15px; background: #f0f0f0; border-radius: 4px;">
                    <p><strong>Important:</strong> After setting the homepage, you need to reset WordPress permalinks.</p>
                    <p>This will automatically redirect you to the permalinks settings page.</p>
                </div>

                <div style="display: flex; gap: 10px; margin-top: 20px;">
                    <button type="button" id="ai-set-homepage" style="
                        padding: 10px 20px;
                        background: #2271b1;
                        color: white;
                        border: none;
                        border-radius: 4px;
                        cursor: pointer;
                    ">Set Homepage & Reset Permalinks</button>

                    <button type="button" id="ai-skip-popup" style="
                        padding: 10px 20px;
                        background: #ccc;
                        color: #333;
                        border: none;
    ...
    `;
            ...
            // Add event listeners
            document.getElementById('ai-set-homepage').addEventListener('click', setHomepage);
            document.getElementById('ai-skip-popup').addEventListener('click', function () {
                document.getElementById('ai-homepage-popup').remove();
            });
        }

        function setHomepage() {
            const select = document.getElementById('ai-homepage-select');
            ...
            // Show loading
            const button = document.getElementById('ai-set-homepage');
            const originalText = button.textContent;
            button.textContent = 'Processing...';
            button.disabled = true;

            // AJAX request to set homepage
            const data = new FormData();
            data.append('action', 'ai_set_homepage');
            data.append('page_id', pageId);
            data.append('nonce', '<?php echo wp_create_nonce("ai_set_homepage"); ?>');
            ...
            fetch('<?php echo admin_url("admin-ajax.php"); ?>', {
                ...
                .then(data => {
                    if (data.success) {
                        // Redirect to permalinks page
                        window.location.href = '<?php echo admin_url("options-permalink.php"); ?>';
                    } else {
                        alert('Error: ' + (data.data || 'Failed to set homepage'));
